...
Grid::setup_cells(void) {
	...
	for (int j = 0; j < num_objects; j++) {
		...
		int ixmin = clamp((obj_bBox.p0.x - p0.x) * nx / (p1.x - p0.x), 0, nx - 1);
		int iymin = clamp((obj_bBox.p0.y - p0.y) * ny / (p1.y - p0.y), 0, ny - 1);
		int izmin = clamp((obj_bBox.p0.z - p0.z) * nz / (p1.z - p0.z), 0, nz - 1);
		int ixmax = clamp((obj_bBox.p1.x - p0.x) * nx / (p1.x - p0.x), 0, nx - 1);
		int iymax = clamp((obj_bBox.p1.y - p0.y) * ny / (p1.y - p0.y), 0, ny - 1);
		int izmax = clamp((obj_bBox.p1.z - p0.z) * nz / (p1.z - p0.z), 0, nz - 1);
...
Grid::find_min_bounds(void) {
	...
	for (int j = 0; j < num_objects; j++) {
		...
		if (object_box.p0.x < p0.x)
			p0.x = object_box.p0.x;
		if (object_box.p0.y < p0.y)
			p0.y = object_box.p0.y;
		if (object_box.p0.z < p0.z)
			p0.z = object_box.p0.z;
	}
...
Grid::find_max_bounds(void) {
	...
	for (int j = 0; j < num_objects; j++) {
		...
		if (object_box.p1.x > p1.x)
			p1.x = object_box.p1.x;
		if (object_box.p1.y > p1.y)
			p1.y = object_box.p1.y;
		if (object_box.p1.z > p1.z)
			p1.z = object_box.p1.z;
	}
	...
	for (int i = 0; i < curve_num; i++) {
		...
		for (int k = 0; k <= vertical_steps - 1; k++) {
			for (int m = 0; m <= horizontal_steps - 1; m++) {
				...
				Triangle* triangle_ptr1 = new Triangle(v0, v2, v1, reverse_normal);
				...
				Triangle* triangle_ptr2 = new Triangle(v0, v2, v1, reverse_normal);
	...
	for (int i = 0; i < patches_num; i++) {
		...
		for (int k = 0; k <= vertical_steps - 1; k++) {
			for (int m = 0; m <= horizontal_steps - 1; m++) {
				...
				Triangle* triangle_ptr1 = new Triangle(v0, v2, v1);
				objects.push_back(triangle_ptr1);
				...
				v2 = Point3D(xxx3, yyy3, zzz3);
				Triangle* triangle_ptr2 = new Triangle(v0, v2, v1);
				objects.push_back(triangle_ptr2);
			}
		}
	}
...
Grid::hit(const Ray& ray, float& t, ShadeRec& sr) const {
	...
	while (true) {
		...
		auto tt = ix + nx * iy + nx * ny * iz;
		if (tt < 0 || tt > cells.size() - 1)
		{
			return false;
		}
