...
#include "../Utilities/Maths.hpp"	// for SolveQuartic
...
bool
TorusPartConcave::hit(const Ray& ray, float& tmin, ShadeRec& sr) const {
	if (!bbox.hit(ray))
		return (false);
...
bool
TorusPartConcave::shadow_hit(const Ray& ray, double& tmin) const {
	if (!bbox.hit(ray))
		return (false);
