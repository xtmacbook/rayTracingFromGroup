...
TorusPartConvex& TorusPartConvex::operator = (TorusPartConvex& rhs) {
	if (this == &rhs)
		return (*this);
...
bool
TorusPartConvex::hit(const Ray& ray, float& tmin, ShadeRec& sr) const {
	if (!bbox.hit(ray))
		return (false);
...
TorusPartConvex::shadow_hit(const Ray& ray, double& tmin) const {
	...
	for (int j = 0; j < num_real_roots; j++)
		if (roots[j] > kEpsilon) {
			...
			if ((roots[j] < t) &&
				((theta > theta1) && (theta < theta2)) &&
				(((phi > phi1) && (phi < phi2)) || ((phi + 2 * PI) < phi2))) {
				t = roots[j];
				intersected = true;
			}
		}
